...
const API_BASE = '/api'; 
const delay = ms => new Promise(resolve => setTimeout(resolve, ms)); // Fungsi Jeda Waktu
...
const HOME_SECTIONS = [
    ...
    { title: "Action Hits", queries: ["kimetsu", "jujutsu", "piece", "bleach", "hunter"] },
    { title: "Romance & Drama", queries: ["love", "kanojo", "romance", "heroine"] },
    { title: "School Life", queries: ["school", "gakuen", "classroom"] },
    { title: "Comedy & Chill", queries: ["comedy", "slice of life", "bocchi"] }
];

const GENRE_KEYWORDS = {
    "Action": ["action", "shounen", "fight"],
    "Adventure": ["adventure", "journey", "world"],
    "Comedy": ["comedy", "slice of life", "laugh"],
    "Drama": ["drama", "cry", "love", "romance"],
    "Fantasy": ["fantasy", "magic", "maou"],
    "Isekai": ["isekai", "reincarnation", "world"],
    "Magic": ["magic", "mahou", "witch"],
    "Romance": ["romance", "love", "kanojo"],
    "School": ["school", "gakuen", "classroom"],
    "Sci-Fi": ["sci-fi", "science", "gundam"],
    "Slice of Life": ["slice of life", "daily", "chill"],
    "Sports": ["sports", "soccer", "blue lock"]
};
...
async function fetchWAFollowers() {
    ...
    try {
        ...
        if (match && match[1]) countEl.innerText = match[1];
        else countEl.innerText = "22.2K";
    } catch (err) { countEl.innerText = "22.2K"; }
}
...
function switchTab(tabName) {
    ...
    if (tabName === 'home') {
        ...
        show('developer-view'); document.getElementById('tab-developer').classList.add('active'); fetchWAFollowers();
    }
}
...
// PERBAIKAN: Load Category tanpa spam request
async function loadCategory(genre, btnElement) {
    ...
    try {
        ...
        // Loop satu per satu agar server tidak mengira DDoS
        for (const q of queriesToFetch) {
            ...
            await delay(350); // Jeda 350 milidetik antar request
        }
...
// PERBAIKAN: Load Latest tanpa spam request
async function loadLatest() {
    ...
    try {
        ...
        if (sliderData && sliderData.length > 0) {
            ...
            // Loop ambil detail satu per satu agar tidak diblokir
            (async () => {
        ...
        // Load kategori baris per baris dengan antrean
        for (let i = 1; i < HOME_SECTIONS.length; i++) {
            ...
            for (const q of section.queries) {
                ...
                await delay(350); // Jeda 350 milidetik antar baris kategori
            }
...
async function loadVideo(url) {
    ...
    try {
        ...
        if (data.streams && data.streams.length > 0) {
            ...
            alert('Maaf, stream gagal dimuat karena diblokir oleh server Otakudesu. Coba beberapa saat lagi.');
        }
    } catch (err) { } finally { loader(false); }
}
