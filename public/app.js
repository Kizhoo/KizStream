...
// FIX: Simpan URL episode terakhir untuk retry
...
async function loadVideo(url) {
    ...
    try {
        ...
        // FIX: Tampilkan watch-view dulu sebelum cek stream
        hide('detail-view'); show('watch-view'); hide('bottomNav');
        document.getElementById('video-title').innerText = data.title || 'Memuat...';
        ...
        if (data.streams && data.streams.length > 0) {
            ...
            player.src = '';
            console.error('[loadVideo] Stream kosong. Data:', data);
            const srcUrl = data.sourceUrl || url;
            serverContainer.innerHTML = `
                <div style="width:100%;padding:20px 16px;text-align:center;color:var(--text-muted);">
                    <svg width="44" height="44" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" style="margin-bottom:10px;opacity:0.5"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
                    <p style="margin:0 0 6px;font-weight:700;font-size:15px;color:var(--text)">Stream Tidak Tersedia</p>
                    <p style="margin:0 0 18px;font-size:12px;line-height:1.5;">Proxy gagal mengambil stream.<br>Coba opsi di bawah ini:</p>
                    <div style="display:flex;flex-direction:column;gap:10px;max-width:280px;margin:0 auto;">
                        <button onclick="loadVideo('${url}')" style="background:var(--accent);color:#fff;border:none;padding:12px 20px;border-radius:20px;font-size:14px;cursor:pointer;font-weight:600;">
                            🔄 Coba Lagi
                        </button>
                        <a href="${srcUrl}" target="_blank" style="background:#1db954;color:#fff;border:none;padding:12px 20px;border-radius:20px;font-size:14px;cursor:pointer;font-weight:600;text-decoration:none;display:block;">
                            🌐 Buka di Browser
                        </a>
                        <button onclick="backToDetail()" style="background:var(--surface);color:var(--text);border:1px solid var(--border);padding:12px 20px;border-radius:20px;font-size:14px;cursor:pointer;">
                            ← Episode Lain
                        </button>
                    </div>
                    <p style="margin:14px 0 0;font-size:11px;opacity:0.5;">Tap Buka di Browser untuk nonton<br>langsung di samehadaku.how</p>
                </div>`;
        }
    } catch (err) {
        // FIX: Jangan silent! Log error dan tampilkan ke user
        console.error('[loadVideo] Error:', err);
        show('watch-view'); hide('detail-view'); hide('bottomNav');
        const serverContainer = document.getElementById('server-options');
        if (serverContainer) {
            serverContainer.innerHTML = `
                <div style="width:100%;padding:20px;text-align:center;color:var(--text-muted);">
                    <p style="color:var(--text);font-weight:600;">Gagal memuat stream</p>
                    <p style="font-size:13px;">${err.message}</p>
                    <button onclick="loadVideo('${url}')" style="background:var(--accent);color:#fff;border:none;padding:10px 24px;border-radius:20px;font-size:14px;cursor:pointer;margin-right:8px;">
                        🔄 Coba Lagi
                    </button>
                    <button onclick="backToDetail()" style="background:var(--surface);color:var(--text);border:1px solid var(--border);padding:10px 24px;border-radius:20px;font-size:14px;cursor:pointer;">
                        ← Kembali
                    </button>
                </div>`;
        }
