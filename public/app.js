    ...
    try {
        ...
        if (data.streams && data.streams.length > 0) {
            ...
            serverContainer.innerHTML = `
                <div style="width:100%;padding:30px 16px;text-align:center;color:var(--text-muted);">
                    <svg width="44" height="44" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" style="margin-bottom:12px;opacity:0.4"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
                    <p style="margin:0 0 8px;font-weight:700;font-size:15px;color:var(--text)">Stream Tidak Tersedia</p>
                    <p style="margin:0 0 20px;font-size:13px;">Episode ini belum tersedia di server manapun.<br>Coba episode lain atau kembali nanti.</p>
                    <div style="display:flex;justify-content:center;gap:10px;flex-wrap:wrap;">
                        <button onclick="loadVideo('${url}')" style="background:var(--accent);color:#fff;border:none;padding:11px 22px;border-radius:20px;font-size:14px;cursor:pointer;font-weight:600;">
                            🔄 Coba Lagi
                        </button>
                        <button onclick="backToDetail()" style="background:var(--surface);color:var(--text);border:1px solid var(--border);padding:11px 22px;border-radius:20px;font-size:14px;cursor:pointer;">
                            ← Episode Lain
                        </button>
                    </div>
                </div>`;
