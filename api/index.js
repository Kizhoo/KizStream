...
async function download(link) {
    try {
        ...
        // 1. GET request pertama untuk mengambil HTML dan COOKIES
        const pageRes = await axios.get(`${PROXY}${targetUrl}`, { headers: getHeaders(BASE_URL) });
        
        // Tangkap cookies dari response header (jika Samehadaku memberikannya)
        let sessionCookies = '';
        if (pageRes.headers['set-cookie']) {
            sessionCookies = pageRes.headers['set-cookie'].map(c => c.split(';')[0]).join('; ');
        }
        ...
        // 2. Lakukan request AJAX ke setiap server dengan membawa COOKIES
        for (let i = 0; i < servers.length; i++) {
            ...
            if (post && nume) {
                try {
                    ...
                    if (sessionCookies) {
                        ajaxHeaders['Cookie'] = sessionCookies;
                    }
                    ...
                    console.log(`Gagal memuat ${serverName}`);
                }
            }
        }

        // 3. Fallback Darurat: Jika AJAX diblokir, cari iframe yang mungkin menempel langsung di HTML
        if (streams.length === 0) {
            const directIframe = $('.entry-content iframe').attr('src');
            if (directIframe) streams.push({ server: 'Default', url: directIframe });
        }
