...
// Kita KEMBALIKAN ke proxy Caliph khusus untuk mengambil tampilan halaman (GET)
// Karena proxy ini sudah terbukti berhasil lolos dari blokir awal Samehadaku
...
// --- BAGIAN GET (HOMEPAGE & PENCARIAN) MENGGUNAKAN PROXY CALIPH ---
...
// --- BAGIAN VIDEO MENGGUNAKAN DIRECT REQUEST (TANPA PROXY) ---
async function download(link) {
    try {
        const targetUrl = link.startsWith('http') ? link : `${BASE_URL}${link}`;
        
        // Ambil elemen HTML halaman episode pakai proxy
        ...
        for (let i = 0; i < servers.length; i++) {
            ...
            if (post && nume) {
                try {
                    ...
                    // PENTING: Tembak request POST langsung ke Samehadaku (TANPA PROXY CALIPH)
                    // Karena proxy Caliph kemungkinan memblokir payload POST kita.
                    ...
                    console.error(`Gagal akses server: ${serverName}`);
                }
            }
        }

        // Fallback: Jika Samehadaku menaruh iframe langsung di HTML tanpa perlu diklik
        if (streams.length === 0) {
            const directIframe = $('.entry-content iframe').attr('src') || $('iframe[src*="player"], iframe[src*="embed"]').attr('src');
            if (directIframe) streams.push({ server: 'Server Cadangan', url: directIframe });
        }
