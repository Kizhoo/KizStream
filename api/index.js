...
async function download(link) {
  ...
  for (const li of serverElements.toArray()) {
    ...
    // Coba TANPA nonce dulu (terbukti berhasil di debug),
    // fallback ke dengan nonce jika response "0"
    const bodyParamsNoNonce = { action: 'player_ajax', post, nume, type };
    const bodyParamsWithNonce = { action: 'player_ajax', post, nume, type };
    if (nonce) bodyParamsWithNonce.nonce = nonce;

    const bodyNoNonce = new URLSearchParams(bodyParamsNoNonce).toString();
    const bodyWithNonce = new URLSearchParams(bodyParamsWithNonce).toString();

    const extraHeaders = {
      'Referer': targetUrl,
      'Origin': ajaxDomain,   // FIX: pakai domain dari URL target, bukan hardcode BASE_URL
      'Cookie': cookieStr,
      // FIX: Host header v1.samehadaku.how dihapus — domain aktif adalah v2!
    };

    console.log(`[Stream] Trying server: "${name}" post=${post} nume=${nume} type=${type}`);
    try {
      // Coba tanpa nonce dulu
      let r = await postWithFallback(ajaxUrl, bodyNoNonce, extraHeaders);
      const rawCheck = String(r.data).trim();
      // Jika response "0" (AJAX gagal), coba lagi dengan nonce
      if ((rawCheck === '0' || rawCheck === '') && nonce) {
        console.log(`[Stream] ⚠️ Got "0" without nonce, retrying with nonce...`);
        r = await postWithFallback(ajaxUrl, bodyWithNonce, extraHeaders);
      }
